...
async function main() {
  ...
  if (!cwd || !sessionId || !transcriptPath)
    process.exit(0);
...
`);
  ...
  try {
    ...
    if (result.memories_stored > 0 || result.duplicates_bumped > 0) {
      process.stderr.write(`[apsolut-cortex] compress-worker: ${result.memories_stored} new memories, ${result.duplicates_bumped} dup-bumped (cursor=${result.new_cursor})
`);
    }
  } catch (e) {
